...
type InnerProvidesNestedList<T> = <Results extends { results: { id: unknown }[] }, Error extends FetchBaseQueryError>(
  results: Results | undefined,
  error: Error | undefined
) => CacheList<T, Results['results'][number]['id']>
...
export const providesNestedList =
  ...
  (results, error) => {
    ...
    if (results) {
      // successful query
      return [{ type, id: 'LIST' }, ...results.results.map(({ id }) => ({ type, id } as const))]
    }
    // Received an error, include an error cache item to the cache list
    return concatErrorCache([{ type, id: 'LIST' }], error)
  }
...
export const cacheByIdArgProperty =
  <T extends string>(type: T) =>
  <Arg extends { id: unknown }, Result = undefined, Error = undefined>(result: Result, error: Error, arg: Arg): readonly [CacheItem<T, Arg['id']>] | [] =>
